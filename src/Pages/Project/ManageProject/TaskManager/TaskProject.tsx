...
const TaskProject = () => {
  ...
  const [isModifTeamApplied, setIsModifTeamApplied] = useState<boolean>(false);
  const [refresh, setRefresh] = useState(false);
  const [isPhasesLoading, setIsPhasesLoading] = useState<boolean>(false);


  const fetchProject = async (isInitialLoad: boolean = false) => {
    if (projectId) {
      const project = await getProjectById(projectId);
      setProjectData(project);
      // stop phases loading when fresh data has arrived
      setIsPhasesLoading(false);
      const phase = project?.listPhases?.sort(
        (a: { rank: number }, b: { rank: number }) => a.rank - b.rank
      );
      if (isInitialLoad && phase?.[0]?.id) {
        navigate(`/gmp/project/task/${projectId}/${phase?.[0]?.id}`);
      }
    }
  };

  const modifTeamInit = useRef<boolean>(false);
  const refreshInit = useRef<boolean>(false);
  ...
  // run when isModifTeamApplied changes, but skip first mount
  useEffect(() => {
    if (!modifTeamInit.current) {
      modifTeamInit.current = true;
      return;
    }
    fetchProject(false);
    setIsModifTeamApplied(false);
  ...
  // run when refresh toggles, but skip first mount
  useEffect(() => {
    if (!refreshInit.current) {
      refreshInit.current = true;
      return;
    }
    fetchProject(false);
  }, [refresh]);
