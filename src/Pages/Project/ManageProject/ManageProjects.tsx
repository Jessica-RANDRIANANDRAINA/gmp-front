...
const getMyHabilitation = async () => {
  ...
  // Fetch ALL projects (no filters, large page size)
  const fetchAllProjects = async () => {
    const decode = decodeToken("pr");
    // Use already fetched habilitation or fetch it again if needed
    const hab = await getAllMyHabilitation();

    // Empty filters = get everything
    const completion = "";
    const criticity = "";
    const priority = "";
    const userSelectedName: string[] = [];

    if (hab?.project?.watchAllProject) {
      const project = await getAllProject(
        1, // pageNumber
        1000, // pageSize (enough for all projects)
        "", // title
        userSelectedName,
        priority,
        criticity,
  ...
  // Initial load: wait for habilitation then fetch all projects
  useEffect(() => {
    if (myHabilitation) {
      fetchAllProjects();
    }
  ...
  // Refetch after archiving (or any other modification)
  useEffect(() => {
    if (isArchiveFinished) {
      fetchAllProjects();
      setIsArchiveFinished(false); // reset the flag
    }
  }, [isArchiveFinished]);

  // Navigate to details / historic / advancement / tasks
  useEffect(() => {
    if (idProjectForDetails !== "") {
      navigate(`/gmp/project/details/${idProjectForDetails}/details`);
    }
