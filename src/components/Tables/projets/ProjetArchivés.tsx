...
const ProjetArchivés = ({
  ...
  if (token) {
    try {
      ...
      setDecodedToken(decoded); // This is correct usage
    } catch (error) {
...
console.log("archiver", archivedProjects);
  const sortedData = archivedProjects?.slice().sort((a: IProjectData, b: IProjectData) => {
  ...
  const project = useMemo(() => {
    return archivedProjects?.flatMap((pr: any) => [
      {
        id: pr.id,
        text: pr.title,
  ...
  //   if (archivedProjects) {
  //     if (projectSelected.length < archivedProjects.length) {
  //       setProjectSelected([]);
  //       archivedProjects.map((p) => setProjectSelected((prev) => [...prev, p.id]));
