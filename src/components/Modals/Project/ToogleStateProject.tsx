...
import { getProjectById, updateProjectState } from '../../../services/Project';
...
const ToogleStateProject = ({
    ...
    const confirmToogleProjectState = async () => {
        setLoading(true)
        try {
            const projectType = type === "Débloquer" || type === "Restaurer" ? "En cours" : type
    ...
    return (
        <Modal
            ...
            header={`${type === "Stand by" ? "Voulez vous vraiment mettre ce projet en stand by ?" : `Voulez vous vraiment ${type} ce projet`}`}
            heightSize="40vh"
            widthSize="medium"
        >
            <ModalBody>
